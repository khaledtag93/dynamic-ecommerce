...
- GF-14 Storefront shell source work fixes dead shared-navigation fragments on non-Home pages or when configurable Home sections are disabled. Three focused PHP tests cover Home, other pages and disabled sections. This newer slice needs branch-head CI and QAS checks; see [Global Foundation Audit](docs/GLOBAL_FOUNDATION_AUDIT_2026-09-26.md).
...
- **Next:** confirm branch-head CI for GF-14, then continue actual Admin/Storefront navigation, conditional UI and role-variant review against the registered routes and shared-shell foundation. When the matching source is on QAS, carry out authenticated EN/AR/mobile/keyboard acceptance before any CLOSED claim. Do not infer this from the route manifest.
