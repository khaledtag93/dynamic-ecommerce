...
- **Storefront Add to Cart live UX (2026-09-25):** primary Add to Cart actions on product cards and Product Details now use a delegated server-confirmed no-reload flow, update the header cart count, preserve validation/stock rules and normal POST fallback, and intentionally leave Buy Now as checkout navigation. Hardening CI #1402 passed at `72b8528` with 383 tests / 2596 assertions. Category Quick View is now connected to the same live flow; its branch-head CI remains pending. QAS acceptance pending; Production unchanged. See `docs/STOREFRONT_ADD_TO_CART_LIVE_UX_2026-09-25.md`.
