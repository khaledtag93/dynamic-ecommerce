...
- GF-13 removed environment/database metadata from public health probes and prevented trace-file writes when the local diagnostics flag is off; source `39c6a87` passed Hardening CI `36251884894`. GF-14 then corrected Storefront shared navigation fragments across pages/disabled Home sections with three focused tests; its newer branch-head CI and QAS gate are tracked in `CURRENT_PHASE.md`.
...
- **QAS automatic operations evidence:** hPanel cron execution is now verified across multiple cycles. Scheduler and queue-worker heartbeats are fresh automatically, `QUEUE_CONNECTION=database`, failed jobs remain `0`, and scheduled Growth / notification escalation / stock-reservation-expiry commands have executed from the Laravel scheduler. A transient single pending heartbeat job can appear because scheduler and bounded queue cron start in the same minute; the next worker cycle consumes it.
...
- **Remaining P0 release gates:** historical credential rotation evidence (OPS-01), Paymob E2E (PAY-01), and database restore rehearsal (OPS-02). OPS-03 automatic scheduler/queue runtime proof is now complete on QAS; Production still needs the same operational setup before promotion.
...
- **Storefront Cart live quantity update (2026-09-25):** quantity changes now update accepted quantity, line total and canonical cart totals without a full-page reload, while preserving server-side ownership/stock rules and a no-JS form fallback. The cart now renders coupon and promotion discounts from separate summary buckets. Arabic live-error copy and focused regression coverage were added. CI/QAS acceptance pending; Production unchanged. See `docs/STOREFRONT_CART_LIVE_UPDATE_2026-09-25.md`.
- **Storefront Cart Coupon live UX (2026-09-25):** coupon apply/remove now use server-confirmed in-place updates for coupon state and cart totals with normal form fallback; coupon validation strings were moved from hard-coded English into translation-aware service messages with Arabic coverage. Coupon usage remains consumed only by the existing checkout path. Focused regression coverage added; Hardening CI #1404 passed at `c0855b9` with 386 tests / 2624 assertions. QAS acceptance pending; Production unchanged. See `docs/STOREFRONT_CART_COUPON_LIVE_UX_2026-09-25.md`.
- **Storefront Checkout bilingual feedback hardening (2026-09-25):** empty-cart, order-success and online-payment handoff flash messages now use translation-aware copy with Arabic coverage; checkout/payment/shipping semantics are unchanged. Audit also records that EGP is currently embedded in the storefront order/shipping domain, so configurable/multi-currency behavior is deferred until a real currency policy is defined end-to-end rather than relabeling UI only. CI/QAS acceptance pending; Production unchanged. See `docs/STOREFRONT_CHECKOUT_BILINGUAL_FEEDBACK_2026-09-25.md`.
- **Storefront Add to Cart live UX (2026-09-25):** primary Add to Cart actions on product cards and Product Details now use a delegated server-confirmed no-reload flow, update the header cart count, preserve validation/stock rules and normal POST fallback, and intentionally leave Buy Now as checkout navigation. Hardening CI #1402 passed at `72b8528` with 383 tests / 2596 assertions; the Category Quick View consistency follow-up passed Hardening CI #1403 at `63f4d43` with 383 tests / 2598 assertions. QAS acceptance pending; Production unchanged. See `docs/STOREFRONT_ADD_TO_CART_LIVE_UX_2026-09-25.md`.
- **Customer Notifications live actions (2026-09-25):** mark-one and mark-all read actions now update in place with server-authoritative ownership/read state, no-JS PATCH fallback, immediate unread-count feedback, and intended navigation after reading actionable notifications. Mark-all no longer loads the full unread collection into memory. Arabic feedback and focused regression coverage were added. CI/QAS acceptance pending; Production unchanged. See `docs/CUSTOMER_NOTIFICATIONS_LIVE_ACTIONS_2026-09-25.md`.
- **Customer Support live UX (2026-09-25):** the customer Help & Support list now uses the shared no-reload pagination pattern with customer isolation verified by Hardening CI #1400 at `058cd0a` (377 tests / 2555 assertions). The follow-up reply composer now supports server-confirmed in-place replies, status/last-updated refresh, no-JS fallback, ownership/Closed-case rules and Arabic error feedback; branch-head CI for the reply batch remains the next gate. Production unchanged. See `docs/CUSTOMER_SUPPORT_LIVE_UX_2026-09-25.md`.
...
- **Permanent mobile-readiness rule (2026-09-25):** Dynamic is planned to gain Android and iPhone apps after the web platform matures. Existing modules should keep business rules outside Blade/browser-only code, preserve stable server-authoritative contracts/idempotency, and remain suitable for a future authenticated API, push notifications, deep links and mobile OAuth. Do not start the app until the current completion pass is closed; see `docs/PRODUCT_DIRECTION_ADDENDUM_2026-09-25.md`.
...
- **Customer Account Statement / movement ledger:** add an Admin customer statement workspace that explains the customer's commercial movement over time from canonical records: orders, captured/paid payments, refunds, returns and future credit/wallet/account adjustments when those modules exist. Include date/type/reference filters, drill-through to source records, debit/credit semantics where applicable, export/print, running balance only when backed by a real ledger, and full auditability. Do not invent a balance from unrelated totals.
- These are planned product requirements, not currently deployed capabilities. Sequence them with identity, support and financial-ledger dependencies rather than interrupting the active verified-reviews slice.
