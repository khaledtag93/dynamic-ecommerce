...
- **Storefront Cart Coupon live UX (2026-09-25):** coupon apply/remove now use server-confirmed in-place updates for coupon state and cart totals with normal form fallback; coupon validation strings were moved from hard-coded English into translation-aware service messages with Arabic coverage. Coupon usage remains consumed only by the existing checkout path. Focused regression coverage added; CI/QAS acceptance pending; Production unchanged. See `docs/STOREFRONT_CART_COUPON_LIVE_UX_2026-09-25.md`.
