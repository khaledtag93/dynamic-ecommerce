...
<div class="admin-page-shell growth-form-page">
    ...
    <form method="POST" action="{{ $segment->exists ? route('admin.growth.segments.update', $segment) : route('admin.growth.segments.store') }}" data-submit-loading data-growth-form>
        @csrf
        @if($segment->exists)
            @method('PUT')
        @endif
