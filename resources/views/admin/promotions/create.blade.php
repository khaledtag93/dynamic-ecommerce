...
<div class="admin-card">
    <div class="admin-card-body">
        <form method="POST" action="{{ $mode === 'edit' ? route('admin.promotions.update', $promotion) : route('admin.promotions.store') }}" data-submit-loading data-admin-section-tabs="promotion-editor" data-confirm-when-field="is_active" data-confirm-when-value="1" data-confirm-title="{{ __('Publish active promotion?') }}" data-confirm-message="{{ __('This promotion will be active in checkout according to its schedule. Review the discount value, eligibility, and dates before saving.') }}" data-confirm-ok="{{ __('Save active promotion') }}">
            @csrf
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const buyXFields = document.querySelectorAll('.promotion-buy-x-field');
    function syncPromotionFields() {
        const isBuyX = type?.value === 'buy_x_get_y';
        buyXFields.forEach(field => {
            field.style.display = isBuyX ? '' : 'none';
            field.querySelectorAll('input').forEach(input => input.disabled = !isBuyX);
        });
    }
    type?.addEventListener('change', syncPromotionFields);
    syncPromotionFields();
});
</script>
@endpush
