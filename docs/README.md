...
| Batch 0: access, product-page trust, demo-data guard | Code on `v42-clean-baseline`: require a limited staff role for promotion; block owner edits in customer management; remove fabricated product counts/reviews and repeated reassurance blocks; restrict demo seed/clear to local/testing/staging in both web and service paths | Awaiting CI for this batch | Not deployed/verified | Not deployed | Explicit owner migration/fallback removal, authentic review workflow, actual content review, QAS visual check. |
...
3. Split the product editor into navigable sections without losing form state; simplify order detail, branding and the default admin dashboard one screen at a time.
